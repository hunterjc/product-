...
exports.mongoDBP = async (req, clientIp) => {
  ...
  try {
    ...
    let findAA = find;
    const myAggregate1 = productTag.aggregate([
      { $match: query },
      {
        $lookup: {
          from: "categories",
    ...
    const data = await productTag.aggregatePaginate(myAggregate1, {
...
exports.edit = async (req, clientIp) => {
  try {
    ...
    let newData = await productTag.findByIdAndUpdate(
      params.id,
