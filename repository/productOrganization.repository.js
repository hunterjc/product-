...
const productOrganization = require("../model/product.organizatin");
exports.mongoDBP = async (req, clientIp) => {
  let params = req.body;
  try {
    params.searchValue = params.searchValue || "name, description";
    params.selectValue =
      params.selectValue || "name, description,createdAt,updatedAt";
    ...
    let findAA = find;
    const myAggregate1 = productOrganization.aggregate([
      { $match: query },
      {
        $lookup: {
          from: "categories",
    ...
    const myAggregate = productOrganization.aggregate([
      { $match: query },
      {
        $lookup: {
          from: "jobcategories",
    ...
    const data = await productOrganization.aggregatePaginate(myAggregate1, {
...
exports.Add = async (req, clientIp) => {
  try {
    ...
    let data = new productOrganization({
      ...params,
    });
    data.save();
...
exports.edit = async (req, clientIp) => {
  try {
    ...
    let newData = await productOrganization.findByIdAndUpdate(
      params.id,
...
exports.remove = async (req, clientIp) => {
  try {
    ...
    let userexist = productOrganization.findOne({ _id: params.id });
    if (!userexist) {
      return { status: 400, message: "data not found" };
    }
    const del = await productOrganization.updateOne(
      {
        _id: params.id,
        deletedAt: null,
      },
